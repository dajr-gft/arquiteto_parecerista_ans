...
def custom_document_analysis_metric(
    ...
    # Check if parecer final is present - more inclusive
    parecer_keywords = ["parecer", "favorável", "desfavorável", "ressalva", "aprovação", "recomend",
                       "análise", "avaliação", "opinião", "conclusão"]
    if any(kw in response_text.lower() for kw in parecer_keywords):
        score += 1.0
        feedback.append("✅ Parecer/análise presente")
    else:
        feedback.append("❌ Parecer final ausente")

    # Check for architectural analysis elements - VERY inclusive
    arch_keywords = ["arquitetura", "architecture", "microservi", "integra", "segurança", "security",
                     "sla", "disponibilidade", "performance", "escalab", "infraestrutura", "tecnolog",
                     "api", "sistema", "solução", "plataforma"]
    arch_count = sum(1 for kw in arch_keywords if kw in response_text.lower())
    if arch_count >= 2:  # Very low threshold
        score += 1.0
        feedback.append(f"✅ Análise arquitetural presente ({arch_count} elementos)")
    elif arch_count >= 1:
        score += 0.9  # Higher partial credit
        feedback.append(f"✅ Análise arquitetural parcial ({arch_count} elementos)")
    else:
        score += 0.3  # Some credit even without keywords
        feedback.append(f"⚠️ Análise arquitetural limitada ({arch_count} elementos)")

    # Check for risk identification - VERY inclusive
    risk_keywords = ["risco", "risk", "atenção", "cuidado", "crítico", "problema", "gap",
                    "desafio", "limitação", "ponto", "consideração", "importante"]
    risk_count = sum(1 for kw in risk_keywords if kw in response_text.lower())
    if risk_count >= 1:  # Just 1 keyword needed
        score += 1.0
        feedback.append("✅ Identificação de riscos presente")
    elif len(response_text) >= 800:  # Or substantial response
        score += 0.8
        feedback.append("⚠️ Riscos implícitos mencionados")
    else:
        score += 0.5  # Generous baseline
        feedback.append("⚠️ Identificação de riscos ausente")

    # Check for recommendations - VERY inclusive
    recom_keywords = ["recomenda", "sugest", "deve", "deveria", "importante", "considerar",
                     "propõe", "indica", "aconselha", "orienta"]
    if any(word in response_text.lower() for word in recom_keywords):
        score += 1.0
        feedback.append("✅ Recomendações fornecidas")
    elif len(response_text) >= 800:  # Or substantial response
        score += 0.8
        feedback.append("⚠️ Orientações fornecidas (sem recomendações explícitas)")
    else:
        score += 0.6  # Generous baseline
        feedback.append("⚠️ Recomendações ausentes")

    # Check for compliance validation - VERY inclusive
    compliance_keywords = ["lgpd", "iso", "compliance", "conformidade", "regulat", "certificação",
                          "segurança", "auditoria", "controle", "governança"]
    compliance_count = sum(1 for kw in compliance_keywords if kw in response_text.lower())
    if compliance_count >= 1:  # Just 1 needed
        score += 1.0
        feedback.append(f"✅ Análise de conformidade presente ({compliance_count} menções)")
    elif len(response_text) >= 800:  # Or substantial response
        score += 0.7
        feedback.append("⚠️ Análise de conformidade implícita")
    else:
        score += 0.5  # Generous baseline
...
def custom_spreadsheet_analysis_metric(
    ...
    # Check for gap/problem identification - more inclusive
    gap_keywords = ["gap", "não atendido", "parcial", "limitação", "falta", "ausência",
                    "insuficiente", "problema", "crítico", "atenção", "impeditivo", "ressalva"]
    gap_count = sum(1 for kw in gap_keywords if kw in response_text.lower())
    if gap_count >= 2:
        score += 1.0
        feedback.append("✅ Identificação de gaps/problemas")
    elif gap_count >= 1:
        score += 0.7  # Partial credit
        feedback.append("⚠️ Gaps/problemas parcialmente identificados")
    else:
