...
class AgentMetrics:
    ...
    def evaluate_success(
        ...
        has_error = (
            "erro" in response_text.lower() or
            "error" in response_text.lower() or
            "falha" in response_text.lower() or
            "fail" in response_text.lower()
        )

        # Check for success indicators
        has_success = (
            "sucesso" in response_text.lower() or
            "success" in response_text.lower() or
            "parecer" in response_text.lower() or
            len(response_text) >= 200  # Substantial response usually means success
        )
        ...
        if expected_success:
            if has_success and not has_error:
                details["pass"] = True
                details["reason"] = "Agent completed task successfully"
                return 1.0, details
            elif has_error:
                details["reason"] = "Agent reported error when success was expected"
                return 0.0, details
            else:
